...
(function () {
  ...
  async function loadReown() {
    ...
    // This static-site build loads AppKit as browser ES modules. A production Vite
    // build can replace these imports with normal npm imports without changing the API.
    const appkitPkg = await import('https://esm.sh/@reown/appkit@latest?bundle');
    const adapterPkg = await import('https://esm.sh/@reown/appkit-adapter-ethers@latest?bundle');
    const networksPkg = await import('https://esm.sh/@reown/appkit@latest/networks?bundle');

    const defineChain = networksPkg.defineChain;
    const EthersAdapter = adapterPkg.EthersAdapter;
    const createAppKit = appkitPkg.createAppKit;
    ...
    const robinhood = defineChain({
      ...
      blockExplorers: { default: { name: 'Robinhood Chain Explorer', url: CONFIG.EXPLORER_URL } }
    });
  ...
  async function connectWithReown() {
    const modal = await loadReown();
    await modal.open();

    // AppKit exposes the connected account/provider through its core instance.
    // Poll briefly because the modal is asynchronous and the user chooses a wallet.
    for (let i = 0; i < 180; i++) {
      await new Promise(function (resolve) { setTimeout(resolve, 500); });
      const connected = typeof modal.getIsConnected === 'function' ? modal.getIsConnected() : false;
      const address = typeof modal.getAddress === 'function' ? modal.getAddress() : null;
      if (connected && address) {
        state.address = address;
        state.source = 'reown';
        state.chainId = typeof modal.getChainId === 'function' ? chainNumber(modal.getChainId()) : CONFIG.CHAIN_ID;
        try { state.provider = typeof modal.getWalletProvider === 'function' ? modal.getWalletProvider() : null; } catch (e) { state.provider = null; }
        await ensureTargetChain(state.provider || { request: function (args) { return modal.request(args); } }).catch(function () {});
        if (state.provider) await bindProviderEvents(state.provider);
        await refresh().catch(function () {});
        saveState();
        updateWalletUI();
        updateProfileUI();
